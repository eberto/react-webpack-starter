...
export class TodoService implements ITodoService {
    ...
    public fetchTodos(): Promise<Array<Todo>> {
        ...
        return new Promise<Array<Todo>>((resolve: (value?: Array<Todo>) => void, reject: (errors?: any) => void) => {
            ...
            .then(() => this.store.dispatch(state => state.isFetching = false))
    ...
    public deleteTodo(todoId: number): Promise<IAppState> {
        this.store.dispatch((state: IAppState) => state.todos.filter((t: Todo) => t.id === todoId)[0].isDeleting = true);
    ...
    public toggleTodo(todo: Todo): Promise<IAppState> {
        this.store.dispatch((state: IAppState) => state.todos.filter((t: Todo) => t.id === todo.id)[0].isToggling = true);
